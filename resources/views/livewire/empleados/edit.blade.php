                ...
                <label for="phone" class="block space-y-1.5">
                    ...
                    <section class="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4 sm:p-5">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            <div>
                                <h2 class="text-base font-bold text-slate-900">Historial de cargos</h2>
                        ...
                        <div class="mt-5 grid gap-4 sm:grid-cols-2">
                            <label for="job_title" class="block space-y-1.5">
                                ...
                                <input id="job_title" type="text" wire:model="job_title" maxlength="100" class="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
                            ...
                            <label for="position_effective_from" class="block space-y-1.5">
                                ...
                                <input id="position_effective_from" type="date" wire:model="position_effective_from" class="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
                                @error('position_effective_from') <p role="alert" class="text-sm font-medium text-red-700">{{ $message }}</p> @enderror
                            </label>
                        </div>
                        ...
                        <label for="position_reason" class="mt-4 block space-y-1.5">
                            ...
                            <input id="position_reason" type="text" wire:model="position_reason" maxlength="255" placeholder="Ej. Promoción interna" class="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
                            @error('position_reason') <p role="alert" class="text-sm font-medium text-red-700">{{ $message }}</p> @enderror
                        </label>

                        <div class="mt-5 border-t border-emerald-100 pt-4">
                            <h3 class="text-sm font-bold text-slate-900">Línea de tiempo</h3>
                            @if ($positionAssignments->isNotEmpty())
                                <ol class="mt-3 space-y-3">
                                    @foreach ($positionAssignments as $assignment)
                                        <li class="relative rounded-xl border border-emerald-100 bg-white px-4 py-3">
                                            ...
                                            <p class="mt-1 text-sm text-slate-600">
                                                {{ $assignment->effective_from->format('d/m/Y') }}–{{ $assignment->effective_to?->format('d/m/Y') ?? ($assignment->effective_from->gt(now()->startOfDay()) ? 'en adelante' : 'actualidad') }}
                                            </p>
                            ...
                            @else
                                <p class="mt-3 text-sm text-slate-600">Todavía no hay movimientos de cargo registrados.</p>
                            @endif
                        </div>
                    </section>
