...
<body class="bg-gray-50 text-gray-900 antialiased">
    ...
    >
        @auth
            <aside class="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col border-r border-slate-200 bg-slate-950 text-slate-200 xl:flex" aria-label="Navegación principal">
                <div class="flex h-20 shrink-0 items-center border-b border-white/10 px-6">
                    <a href="{{ route('dashboard') }}" class="flex min-w-0 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950" aria-label="Ir al panel">
                        ...
                        <span class="truncate text-lg font-bold tracking-tight text-white">{{ config('app.name', 'Nomina') }}</span>
                    </a>
                </div>

                <nav class="flex-1 overflow-y-auto px-4 py-6" aria-label="Secciones">
                    <p class="px-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Principal</p>
                    <div class="mt-2 space-y-1">
                        @foreach ($primaryNavigation as $item)
                            <a href="{{ route($item['route']) }}"{!! $item['active'] ? ' aria-current="page"' : '' !!} class="flex items-center rounded-xl px-3 py-2.5 text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 {{ $item['active'] ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">
                                {{ $item['label'] }}
                            </a>
                        @endforeach
                    </div>
                    ...
                    @if ($managementNavigation !== [])
                        <div
                            class="mt-8"
                            x-data="{ open: @js($managementActive) }"
                        ...
                        >
                            <button
                                ...
                                class="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 transition hover:bg-white/5 hover:text-slate-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"
                            >
                                Gestión
                            ...
                            <div id="management-disclosure-panel" x-show="open" style="display: none;" class="mt-1 space-y-1">
                ...
                <div class="shrink-0 border-t border-white/10 p-4">
                    <div class="flex items-center gap-3 rounded-xl bg-white/5 px-3 py-3">
                        <span class="grid size-9 shrink-0 place-items-center rounded-full bg-indigo-500 text-sm font-semibold text-white">
                            {{ mb_strtoupper(mb_substr($user->email, 0, 1)) }}
                        </span>
                        <span class="min-w-0">
                            <span class="block truncate text-sm font-medium text-white">{{ $user->email }}</span>
                            <span class="block truncate text-xs text-slate-400">{{ $accountContextLabel }}</span>
                        </span>
                    </div>
                </div>
            </aside>
